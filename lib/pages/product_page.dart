...
import 'package:flutter_nike_shop/UiItems/my_bottom_navigation_bar.dart';
import 'package:flutter_nike_shop/classes/functions.dart';

import '../model.dart';
import 'the_bottom_sheet.dart';

// ProductPage
class ProductPage extends StatefulWidget {
  final ProductShoes shoe;
  final Animation<double> animationValue;
  const ProductPage({Key? key,required this.animationValue,required this.shoe})
      : super(key: key);
  @override
...
late  AnimationController _controller;
        ...
        body: SafeArea(
          child: Stack(
            children: [
                    ...
                    children: [
                      ...
                      AlignTransition(
                        alignment: AlignmentTween(
                            begin: const Alignment(5, 0), end: const Alignment(0, 0))
              ...
              AlignTransition(
                alignment: AlignmentTween(
                    begin: const Alignment(0.92, 1.5), end: const Alignment(0.92, 0.95))
                    .animate(CurvedAnimation(
                    curve: const Cubic(.94, 0, .57, 1.76),
                    reverseCurve: Curves.linear,
                    parent: _controller)),
              ...
              AlignTransition(
                alignment: AlignmentTween(
                    begin: const Alignment(-0.92, 1.5),
                    end: const Alignment(-0.92, 0.95))
                    .animate(CurvedAnimation(
                    curve: const Cubic(.94, 0, .57, 1.76),
                    reverseCurve: Curves.linear,
                    parent: _controller)),
              ...
              const MyBottomNavigationBar(false)
            ],
          ),
        ),
      ),
  ...
  productItems(ProductShoes shoe) {
    debugPrint('Hero Container${widget.shoe.id}');
    return Hero(
  ...
  Widget dotBuilder(int index) {
    return AnimatedContainer(
      ...
      decoration: BoxDecoration(
          ...
          color: currentIndex == index ? Colors.black : const Color.fromRGBO(230, 230, 230, 1)),
    );
  }
}
